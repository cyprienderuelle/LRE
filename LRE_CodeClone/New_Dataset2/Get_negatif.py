...
pos_text = [d['positif'] for d in raw_data]
...
for start_idx in range(0, total_count, CHUNK_SIZE):
    ...
    chunk_pos_texts = pos_text[start_idx:end_idx]
    chunk_embeddings_list = []
    chunk_pos_embeddings_list = []
    
    for i in range(0, len(chunk_texts), BATCH_SIZE):
        batch = [t[:50000] for t in chunk_texts[i : i + BATCH_SIZE]]
        ...
        batch = [t[:50000] for t in chunk_pos_texts[i : i + BATCH_SIZE]]
        inputs = tokenizer(batch, return_tensors="pt", padding=True, truncation=True, max_length=512).to(device)
        ...
        with torch.no_grad():
            ...
            # On n'utilise PLUS la normalisation L2 pour être cohérent avec ton entraînement
            vecs = torch.max(torch.log1p(torch.relu(outputs.logits)) * inputs.attention_mask.unsqueeze(-1), dim=1).values
            chunk_pos_embeddings_list.append(vecs.cpu())
            
        main_pbar.update(len(batch))

    chunk_embeddings = torch.cat(chunk_embeddings_list).to(device)
    chunk_pos_embeddings = torch.cat(chunk_pos_embeddings_list).to(device)
    ...
    with open(output_file, 'a', encoding='utf-8') as f_out:
        ...
        for i in range(len(chunk_texts)):
            ...
            scores = torch.mm(query_vec, chunk_pos_embeddings.t()).squeeze(0)
            ...
            output_obj = {
                ...
                "neg": raw_data[real_neg_idx]['positif'],
    ...
    del chunk_pos_embeddings
    del chunk_pos_embeddings_list
